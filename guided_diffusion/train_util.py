...
class TrainLoop:
    ...
    def save(self):
        ...
        if dist.get_rank() == 0:
            axis = getattr(self.args, "train_axis", "unknown")

            infer_ckpt = {
                "model": self.model.state_dict(),   # ✅ REAL model weights
                "step": self.step + self.resume_step,
                "axis": axis,
            }

            infer_path = bf.join(
                self.get_blob_logdir(),
                f"infer_model_{axis}_{(self.step+self.resume_step):06d}.pt",
            )

            with bf.BlobFile(infer_path, "wb") as f:
                th.save(infer_ckpt, f)

            logger.log(f"[SAVE] Inference model saved to {infer_path}")
